...
def locateMap(box, pion, result, forme):
    locImg = cv.imread("static/img/proto-regions.png")
...
def detColor(img, result):
    ...
    # Etalonnage
    hsv = cv.imread("static/img/proto-REF.png")
...
def detection_main():
    ...
    # Lecture de img
    img = cv.imread("static/img/image.png")
    detColor(img, result)


    with open("game_data/detection_output.json", "w") as f:
        json.dump(result, f)
